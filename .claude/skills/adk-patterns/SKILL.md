---
name: adk-patterns
description: How to build ADK agents in gaia — LlmAgent, tools, SequentialAgent/ParallelAgent, session state, sub-agent delegation. Use when creating or editing any agent in src/gaia/core or src/gaia/agents.
---

# ADK patterns for gaia

- Root Gaia agent = `LlmAgent` built in `core/agent.py`. Delegation to souls goes
...
- Shared data between agents = `Session.state` (the "whiteboard"). That is also
  gaia's short-term memory; long-term is mem0 via `memory/service.py`
