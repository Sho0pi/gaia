...
Runs in a fully isolated ``HOME`` (gaia's HOME_DIR derives from ``Path.home()``), so
the real ``~/.gaia`` is never touched. ``--hold`` keeps serve resident with zero
...
def test_daemon_round_trip(tmp_path: Path) -> None:
    ...
    home = tmp_path / ".gaia"
    pid_file = home / "gaia.pid"
    log_path = tmp_path / "daemon.log"

    with log_path.open("ab") as log:
        proc = subprocess.Popen(
            [sys.executable, "-m", "gaia.cli", "serve", "--hold"],
            env=env,
            stdout=log,
