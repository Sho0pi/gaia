...
Spawn-self + pidfile — ``start`` execs a fresh interpreter (``python -m gaia.cli
serve``) detached via ``start_new_session``; no fork-based daemonization (``fork()``
without ``exec`` is unsafe on macOS and with the native-threaded deps gaia loads).
``start``/``stop``/``status`` import only light config modules — never ``gaia.app``
...
def serve(
    ...
    if pid is not None:
        console().print(f"already running (pid {pid}) — use 'gaia stop' first")
...
def _start(st: CliState) -> int:
    ...
    while time.monotonic() < deadline:
        ...
        if pidfile.read() == proc.pid:  # serve writes its pidfile once startup committed
            ...
            return 0
        time.sleep(_POLL)
    console().print(f"starting (pid {proc.pid}) — startup not confirmed yet, check 'gaia status'")
    return 0
