"""Generate a commented default ``gaia.yaml`` directly from the schema.
...
:class:`~gaia.config.schema.GaiaConfig`'s fields — emitting each field's default value
...
through ``GaiaConfig`` to guarantee it stays valid and in sync.
"""
...
def render_default_yaml() -> str:
    """Render the commented default ``gaia.yaml`` from the live schema."""
    return _HEADER + "\n" + "\n".join(_render_instance(GaiaConfig(), 0)) + "\n"
