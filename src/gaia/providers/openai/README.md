...
This package lets gaia run on a **ChatGPT subscription** (Plus/Pro) instead of a paid
...
…then `gaia model` once to log in (ChatGPT sign-in).
...
1. **OAuth login** (`device_auth.py` + `pkce.py` + `store.py`)
   ...
   - Tokens are stored at `~/.gaia/openai_chatgpt.json` (`0600`, redacted from logs) and
     auto-refreshed.
