"""``godpy llm`` command group: model-provider operations (only ``auth`` for now).
...
from godpy.cli._options import state
...
def auth(
    ...
    """Interactive provider login; stores credentials under ~/.godpy."""
    from godpy.app import run_auth
