...
> I hate python — here is God, the best agent you will ever use.

**God** is an AI agent that spawns task-specific subagents on demand, **stores them for
...
- **Reuse over rebuild.** Every capability leans on a proven library, never a from-scratch
  reinvention.
- **God orchestrator** ([`src/godpy/god`](src/godpy/god)) routes a task to the best subagent.
- **Agent factory + registry** ([`src/godpy/agents`](src/godpy/agents)) builds an ADK agent
